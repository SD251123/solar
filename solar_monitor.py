...
def monitor_solar_status(user_id, user_pw):
  ...
  # 서버 환경 구동을 위한 필수 헤드리스 및 메모리/보안 옵션 보강
  options.add_argument("--headless=new")  # 최신 Chrome Headless 모드 적용
  options.add_argument("--no-sandbox")  # 리눅스 권한 문제 방지
  options.add_argument(
      "--disable-dev-shm-usage"
  )  # /dev/shm 대신 /tmp 사용 (메모리 부족 튕김 방지 핵심)
  options.add_argument("--disable-gpu")  # GPU 가속 비활성화
  options.add_argument(
      "--window-size=1920,1080"
  )  # 브라우저 가상 해상도 고정 (요소 탐색 안정화)
  options.add_argument(
      "user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64)"
      " AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
  )

  driver = None  # finally 구문에서 안전하게 quit() 하기 위해 미리 선언
  ...
  try:
    ...
    print(
        "🔍 실시간 통합 진단 결과 -> 총 감지된 이상 징후:"
        f" {total_issues}건 ({detected_source})"
    )
    ...
    if current_hour in [12, 15, 18] and current_minute < 30:
      ...
      print(
          f"🟢 정기 리포트 시간대({current_hour}시 정각 턴) 도래: 텔레그램으로 정상"
          " 가동 메시지를 전송했습니다."
      )
    else:
      print(
    ...
    # C++ 스택트레이스 대신 에러 핵심 메시지만 한 줄로 전송
